...
@app.post("/memo/")
async def create_memo(memo: MemoInsert, db_conn: Session = Depends(get_connection)):
    """
    - 입력 ( parameters )
...
async def select_memo(
    memo_id: int, memo: MemoUpdate, db_conn: Session = Depends(get_connection)
):
...
async def delete_memo(memo_id: int, db_conn: Session = Depends(get_connection)):
